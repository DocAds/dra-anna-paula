...
/**
 * Botão flutuante do WhatsApp. Ele inverte as cores quando passa por cima de
 * uma superfície escura (.superficie-escura) durante a rolagem: cocoa sobre
 * cocoa deixava o botão praticamente invisível no bloco da Dra. e no rodapé.
 */
export function WhatsAppFloat() {
  ...
  useEffect(() => {
    ...
    const medir = () => {
      ...
      // O que está atrás do centro do botão decide o tema.
      const atras = document.elementsFromPoint(
        caixa.left + caixa.width / 2,
        caixa.top + caixa.height / 2
      );
      setFundoEscuro(
        atras.some((el) => !botao.contains(el) && el.closest(".superficie-escura") !== null)
      );
    };
  ...
  return (
    <motion.button
      ref={ref}
      type="button"
      onClick={() => leadModal.open("float")}
      initial={{ opacity: 0, scale: 0.6 }}
      animate={{ opacity: 1, scale: 1 }}
      transition={{ delay: 1.4, duration: 0.7, ease: [0.22, 1, 0.36, 1] }}
      aria-label="Conversar no WhatsApp"
      className={`fixed bottom-6 right-6 z-40 grid h-14 w-14 place-items-center rounded-full shadow-2xl transition-colors duration-500 ${
        fundoEscuro ? "bg-cream text-cocoa hover:bg-bone" : "bg-cocoa text-bone hover:bg-ink"
      }`}
    >
      <span
        aria-hidden
        className={`absolute inset-0 animate-ping rounded-full transition-colors duration-500 ${
          fundoEscuro ? "bg-cream/30" : "bg-cocoa/30"
        }`}
      />
